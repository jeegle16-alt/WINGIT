...
Key fix:
- DO NOT point both containers to the same model.tar.gz.
- Split the training output model.tar.gz into:
  1) preprocess_model.tar.gz : featurizer.joblib (+ target_transform.json optional) + code/ (inference.py + requirements.txt)
  2) xgb_model.tar.gz        : xgboost-model only

Why:
- XGBoost built-in inference container scans /opt/ml/model and may try to load non-xgb files as a booster.
  If featurizer.joblib is present, it can crash on ping with pickle/xgb load errors.
"""
...
def _download_if_s3(s3_client, path_or_s3: str, local_dir: str) -> str:
    """
    Return local path to a file.
    - If input is s3://..., download into local_dir
    - else return as-is (assumed local)
    """
...
def _make_tar_gz(tar_path: str, base_dir: str, members_relpaths: list[str]):
    """
    Create a tar.gz that contains only specified members (relative paths under base_dir).
    """
...
def _wait_approval(sm, model_package_arn: str, desired: str = "Approved", timeout: int = 180, poll: int = 3):
    start = time.time()
    while True:
        ...
        if time.time() - start > timeout:
            raise RuntimeError(f"Timed out waiting approval '{desired}'. Current='{status}' arn='{model_package_arn}'")
        time.sleep(poll)
...
def main():
    ...
    parser.add_argument("--model-tar", required=True)        # s3 uri (recommended) OR local path
    parser.add_argument("--model-group", required=True)
    parser.add_argument("--region", required=True)
    parser.add_argument("--role-arn", required=True)

    # 2-container inference pipeline images
    parser.add_argument("--preprocess-image-uri", required=True)  # sklearn inference image
    parser.add_argument("--xgb-image-uri", required=True)         # xgboost inference image
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Expected layout from your train.py:
        # extracted/
        #   xgboost-model
        #   featurizer.joblib
        #   target_transform.json (optional)
        #   code/
        #     inference.py
        ...
        if not os.path.exists(xgb_model):
            raise FileNotFoundError(f"Missing xgboost-model in model tar: {xgb_model}")
        if not os.path.exists(fz):
            raise FileNotFoundError(f"Missing featurizer.joblib in model tar: {fz}")
        if not os.path.exists(infer_py):
            raise FileNotFoundError(
                f"Missing code/inference.py in model tar: {infer_py}\n"
                f"-> Ensure train.py copies inference.py into /opt/ml/model/code/"
            )
    ...
    # 6) create model package (two different ModelDataUrl)
    kwargs = dict(
    ...
    if args.auto_approve:
        print("[register] auto-approving:", model_package_arn)
        sm.update_model_package(
            ModelPackageArn=model_package_arn,
            ModelApprovalStatus="Approved",
            ApprovalDescription="Auto-approved by pipeline for end-to-end execution.",
        )
        _wait_approval(sm, model_package_arn, desired="Approved")
