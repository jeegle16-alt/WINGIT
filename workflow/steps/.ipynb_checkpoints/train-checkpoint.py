...
def parse_args():
    ...
    p.add_argument("--featurizer-dir", type=str, default=os.environ.get("SM_CHANNEL_FEATURIZER", "/opt/ml/input/data/featurizer"))
    p.add_argument("--target-transform-dir", type=str, default=os.environ.get("SM_CHANNEL_TARGET_TRANSFORM", "/opt/ml/input/data/target_transform"))
...
def main():
    ...
    try:
        ...
        # --- copy featurizer payload into model dir
        fz_src = Path(args.featurizer_dir) / "featurizer.joblib"
        if not fz_src.exists():
            raise FileNotFoundError(f"featurizer.joblib not found: {fz_src}")
        code_dir = model_dir / "code"
        code_dir.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(str(fz_src), str(model_dir / "featurizer.joblib"))

        # --- copy target_transform.json (optional)
        tt_src = Path(args.target_transform_dir) / "target_transform.json"
        if tt_src.exists():
            shutil.copyfile(str(tt_src), str(model_dir / "target_transform.json"))
        
        # training container has your source under /opt/ml/code (entry_point/source_dir 업로드됨)
        src_infer = Path("/opt/ml/code") / "inference.py"
        if not src_infer.exists():
            raise FileNotFoundError("inference.py not found in /opt/ml/code. Make sure it's in workflow/steps/ before training.")
        shutil.copyfile(str(src_infer), str(code_dir / "inference.py"))
        # --- include inference requirements so sklearn inference can pip install deps (e.g., xgboost)
        req_src = Path("/opt/ml/code") / "requirements_inference.txt"
        if req_src.exists():
            shutil.copyfile(str(req_src), str(code_dir / "requirements.txt"))
            print("[train] copied inference requirements -> /opt/ml/model/code/requirements.txt")
        else:
            print("[train] requirements_inference.txt not found in /opt/ml/code (skip)")
